...
########################################
# Tested with the principals file generated from Ambari 1.7.0.
#
...
cat $1 | while read next; do
	...
	kadmin.local -q "xst -norandkey -k /tmp/keytabs/$HOST/$KEYTAB $PRIN"
        ...
        ssh -n $HOST 'if [ ! -d "/etc/security/keytabs" ]; then mkdir -p /etc/security/keytabs/; chown root:hadoop /etc/security/keytabs; chmod 750 /etc/security/keytabs; fi'
